# AGENT PROMPT — SPRINT 31 (Çerez politikası popup, B13)

> Uygulayıcı: yerel Qwen (OpenHands), klonda. SADECE kod yaz. Git YOK. Önce `vasi-web/CLAUDE.md`'yi oku.
> Her mini-task (MT) için YENİ konuşma. Sırayla MT1 → MT2 → MT3.
> Kurallar: SADECE tarif edilen dosyalara dokun; `any` yok; hardcode renk yok (`var(--...)`); dosyayı önce KENDİN oku. Edge: bileşen, route değil.

---

## MT1 — YENİ dosya: vasi-web/src/components/CookiePolicyModal.tsx

Tam içerik (birebir oluştur):
```tsx
'use client';
...
## MT2 — layout.tsx mount + CookieConsent.tsx tetikleyici

### 2a) vasi-web/src/app/layout.tsx
Şu satırı bul:
```tsx
...
Sonra şu satırı bul:
```tsx
      <body className={font.variable}>{children}<CookieConsent /></body>
```
ŞUNUNLA değiştir:
```tsx
      <body className={font.variable}>{children}<CookieConsent /><CookiePolicyModal /></body>
```

### 2b) vasi-web/src/components/CookieConsent.tsx
Şu satırı bul ve SİL (artık kullanılmıyor):
```tsx
...
## MT3 — page.tsx footer linki

vasi-web/src/app/page.tsx — şu TEK satırı bul (footer "Hukuki" sütununda):
```tsx
...
```
> page.tsx'te `Link` BAŞKA yerlerde kullanılıyor — importu SİLME, başka satıra dokunma.

---

## Bitiş
- Dokunulan dosyalar: `CookiePolicyModal.tsx` (yeni), `layout.tsx`, `CookieConsent.tsx`, `page.tsx`.
- `cerez-politikasi/page.tsx` silme işini YAPMA — Claude transfer sırasında yapacak.
- Bitince değişen dosya listesi + kısa diff özeti ver.
