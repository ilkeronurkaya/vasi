...
export default function UpgradePage() {
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto', padding: '32px' }}>
      <h1 style={{ fontSize: '22px', fontWeight: '700', letterSpacing: '-0.01em', color: 'var(--cream)' }}>Plan Karşılaştırma</h1>
      <div style={{ display: 'flex', gap: '32px' }}>
        {PLANS.map(plan => (
          <div key={plan.key} style={{
            ...
            <button className="btn btn-primary" style={{ width: '100%', marginTop: '16px', opacity: currentPlan === plan.key ? 0.5 : 1 }} disabled={currentPlan === plan.key} onClick={() => { /* TODO: Implement upgrade logic */ }}>Yükselt</button>
          </div>
        ))}
      </div>
