...
const Messages = () => {
    ...
    return (React.createElement("div", { style: { padding: '24px' } },
        React.createElement("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '36px' } },
            React.createElement("h1", { style: { fontSize: '22px', fontWeight: 700, color: 'var(--cream)', letterSpacing: '-0.01em', margin: 0 } }, t.page_title),
        ...
        loading ? (React.createElement("p", { style: { color: 'var(--mist)', fontSize: '14px' } }, "Y\u00FCkleniyor...")) : messages.length === 0 ? (React.createElement("div", { style: {
            ...
            React.createElement("button", { onClick: () => router.push('/messages/new'), className: "btn btn-primary" }, t.create_first_message_button))) : (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: '12px' } }, messages.map((msg, index) => {
            const status = STATUS_LABELS[msg.status] ?? STATUS_LABELS.draft;
            return (React.createElement("div", { key: index, onClick: () => router.push(`/messages/${msg.id}`), style: {
