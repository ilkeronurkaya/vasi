...
| B14 | P1 | **Dil tercihi tutarsız / sıfırlanıyor.** Dashboard ayarlarından dil değişince uygulanıyor; ama kullanıcı çıkış yapıp landing'de dil değiştirip tekrar girince dil sıfırlanıyor. Kök neden: dashboard i18n (`lib/i18n.ts`) yalnız `tr`/`en` destekler (`getLang` değer `en` değilse `tr`'ye düşer); landing (`page.tsx`) 6 dil yazıyor aynı `vasi_lang` anahtarına → DE/FR/ES/AR seçilince dashboard `tr`'ye sıfırlıyor. Sunucu tarafı per-user dil YOK. İstenen (iko): dil **kalıcı saklansın** ve **yalnız dashboard ayarlarından** değişsin. | (06-19) | S32 (kapsam netleşince) | Açık |
