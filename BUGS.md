...
|----|--------|----------|---------|--------------|-------|
| B12 | P0 | **E-posta teslimatı yalnız hesap sahibine gidiyor.** Admin OTP (hesap sahibi olmayan adresler) + kullanıcı mesaj teslimatı + diğer OTP'ler ulaşmıyor. Kök neden: `EMAIL_FROM=Vasi <onboarding@resend.dev>` (Resend paylaşımlı TEST göndericisi) + doğrulanmış domain yok → Resend yalnız hesap sahibine (`ilkeronurkaya@gmail.com`) yollar. Kanıt (06-19): hesap sahibine diagnostik gönderim `200`+id döndü; key/kod ÇALIŞIYOR. Çözüm: Resend'de domain doğrula (DNS kayıtları) + `EMAIL_FROM`'u o domaine çevir + sonra key rotate. **iko dış aksiyonu (DNS) gerektirir;** kod tarafı küçük (EMAIL_FROM). **ERTELENDİ: iko'da henüz domain yok — proje sonrası domain alınınca yapılacak.** | (06-19) | proje sonrası (domain alınınca) | Açık (ertelendi) |
| B15 | P2 | **Çerez bar'ı dil değişiminde Türkçe kalıyor.** Dil değiştirilince (settings/landing) çerez onay bar'ı çevrilmiyor; sabit Türkçe kalıyor. Kök neden: `CookieConsent.tsx` kendi `useState('tr')` + yalnız mount'ta `vasi_lang` okuyan `useEffect` kullanıyor; `vasi-lang-change` event'ine abone DEĞİL (diğer bileşenler `useLang`/`useSyncExternalStore` ile reaktif). 6 dilli `COPY` zaten var. Fix: `useLang()` kullan (`COPY[lang]`) veya lang-change/storage event'lerine abone ol. | (06-20) | S33 | Açık |
| B16 | P2 | **Landing footer linkleri sayfasız (`href="#"`).** Ürün (Özellikler/Fiyatlandırma/Güvenlik/Yol Haritası), Şirket (Hakkımızda/Blog/Kariyer/İletişim), Hukuki (Gizlilik Politikası/Kullanım Şartları/KVKK) linkleri hiçbir yere gitmiyor (Çerezler modal yapıldı). En kritik: Hukuki üçlü (gerçek yükümlülük). Kapsam/öncelik iko ile netleşecek → S33 planı. | (06-20) | S33 | Açık |
...
|----|--------|----------|-------|--------|
| B14 | P1 | Dil tercihi sıfırlanıyordu — landing 6 dil yazıyor `vasi_lang`'a, dashboard i18n yalnız tr/en (`getLang` `en` değilse `tr`); sunucu tarafı per-user dil yoktu. | Faz 1: migration `0020` `users.language`; `PATCH /me/language` (OTP'siz, 6 dil doğrulama) + `/me` `language` döner; dashboard login'de DB dilini uygular, settings değişimi DB'ye yazar → landing artık ezmez. smoke 61/61. (Faz 2: 6 dil i18n + Faz 3: DE/FR/ES/AR çeviri = ROADMAP, ayrı). | S32 |
| B13 | P1 | Çerez politikası ayrı route'tu; girişten sonra açılıp "Ana sayfaya dön"e basınca landing'e atıyor → kullanıcı uygulamadan çıkıyordu. | `CookiePolicyModal` (event-driven `vasi-open-cookie-policy`, 6 dil + AR RTL, tasarım token'ları); layout'ta bir kez mount; çerez banner'ı + landing footer linki modal açar; `/cerez-politikasi` route'u silindi. tsc/lint 0; yerel ajan yazdı, Claude doğruladı (MT1'de 2 çeviri tipo yakalandı+düzeltildi). iko doğruladı. | S31 |
