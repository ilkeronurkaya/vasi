...
async function create(env, userId, data) {
...
async function update(env, id, userId, data) {
    const { title, message_type, content_text } = data;
    const stmt = env.DB.prepare(`
...
async function remove(env, id, userId) {
    const stmt = env.DB.prepare(`
