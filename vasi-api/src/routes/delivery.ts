...
const deliveryRoutes = new Hono<{ Bindings: Env; Variables: { userId: string } }>()
...
deliveryRoutes.post('/:id/schedule', async (c) => {
  ...
  if (!body.scheduled_at) return c.json({ error: 'scheduled_at zorunlu', code: 'VALIDATION_ERROR' }, 400)
