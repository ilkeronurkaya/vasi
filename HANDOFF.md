...
_Tarih: 2026-06-15 · Bu dosya her oturum başında okunur; durumu git ile çapraz doğrula: `git log --oneline -8 --all`_
...
> İlk gerçek e-posta 2026-06-11'de teslim edildi (Resend). Smoke: **58/58 yeşil** (S25 sonrası da). **Sprint 20–25 KAPANDI** (S25 `sprint-25-i18n`'de push'lu; main'e HENÜZ merge EDİLMEDİ — S26 öncesi FF gerek). Yol haritası + bug listesi ayrı dosyalarda: **`ROADMAP.md`** (sprint planı), **`BUGS.md`** (yaşayan bug listesi). Sıradaki: **S26 — i18n turu 2** (kalan kullanıcı sayfaları; sıra yeniden numaralandı, güvenlik → S27).
>
...
1. ~~S25 — i18n + ayrı admin~~ **KAPANDI (06-15, `sprint-25-i18n` push'lu).** main'e merge bekliyor (S26 öncesi FF + klon tazele).
2. **S26 (SIRADAKİ) — i18n turu 2.** EN'de hâlâ TR kalan kullanıcı sayfaları: mesaj listesi (`/messages`), sihirbaz (`/messages/new` + `/messages/[id]` + `.../schedule`), settings gövdesi, dashboard durum etiketleri (`STATUS_LABELS` + "alıcı·tarih"), `/upgrade`, `/verify-email`. **Admin paneli HARİÇ.** S25 altyapısı (`lib/i18n.ts`) üzerine — yeni `DICT` anahtarları + `t()` dönüşümleri. Tasarım+promt çıkarılacak: `SPRINT_26_I18N2.md` / `AGENT_PROMPT_SPRINT_26.md`.
3. Sonra: **S27** güvenlik+OTP&UX+lint (B5 P0, B6 a–c, B7-B9, B2, B3) → **S28** SMS(NetGSM)+SMS-OTP(B6d)+cookie → **S29** OAuth. Bkz. `ROADMAP.md`.
4. Açık buglar: **B5 (P0 — cross-context OTP açığı)**, B6 (şifre doğrulama/politika), B7-B9 (UX), B1 (premium seed hash), B2 (Yeni Paket 0-default), **B3 (next build ESLint — prod bloke)**, B4 (smoke :3000). Bkz. `BUGS.md`.
5. Resend domain doğrulama → key rotate · gerçek İyzico sandbox (merchant hesabı) · canlıya çıkış (wrangler deploy + Pages).
