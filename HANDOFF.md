...
_Tarih: 2026-06-17 · Bu dosya her oturum başında okunur; durumu git ile çapraz doğrula: `git log --oneline -8 --all`_
...
> İlk gerçek e-posta 2026-06-11'de teslim edildi (Resend). Smoke: **58/58 yeşil**. **Sprint 20–28 KAPANDI.** Yol haritası + bug listesi ayrı dosyalarda: **`ROADMAP.md`** (sprint planı), **`BUGS.md`** (yaşayan bug listesi). Sıradaki: **ROADMAP'ten S29** seç (açık buglar: yalnız **B6d** OTP SMS — NetGSM maliyeti, ertelendi). Askıdaki bloker: İyzico gerçek sandbox (merchant hesabı).
>
> **2026-06-17 İŞ AKIŞI DEĞİŞTİ (iko kararı):** Claude artık **tüm git işlemlerini kendi yapıyor** (branch/commit/merge/push — eski "git hepsi iko'da" kuralı KALKTI) ve **Docker + LM Studio'yu shell'den sürüyor** (izin verildi). Dev stack Docker değil: API `pnpm --filter vasi-api dev` (wrangler dev :8787, local D1 `.wrangler/state`), web `pnpm --filter vasi-web dev` (:3000, `next.config` `/api/v1/*`→`localhost:8787` rewrite). Smoke kendi izole wrangler'ını (:8788, temp DB) kurar; Python **`uv run --python 3.12`** (sistem python3.9 `str|None` annotation'da patlar).
>
> **2026-06-17 Sprint 28 (`sprint-28-stability`, main'e FF merge `028295f`) — stabilite KAPANDI.** Claude doğrudan yazdı+çalıştırdı+doğruladı (yerel ajan kullanılmadı, hepsi küçük/riskli). **B4:** `api_smoke.py` `req()` başarı yolu `json.loads` try/except → non-JSON `(status,{})`; `:3000` açık VE kapalı 58/58. **B1:** `migrations/0019` premium seed hash'i gerçek PBKDF2'ye `UPDATE` (+`0010` taze DB) → `/auth/login` premium **200**+token, yanlış şifre 401 (runtime doğrulandı). **B11:** `public.ts` pricing cache 300→30sn. **B10 (en hacimli):** landing fiyatı `/public/pricing`'ten **tam dinamik** (`plans.map`, N plan); sabit kart + yanlış `d.pricing` alanı kaldırıldı; featured = `{premium,personal}` slug; grid `.pricing-grid--dynamic` (`auto-fit minmax(260px,1fr)`, globals.css) → N plana ortalanmış responsive; 6 dile `plan_msgs/plan_recips/plan_cta`. **RUNTIME DERS:** canlı DB'de 3 plan vardı (free/medium/premium), seed'in beklediği 2 (free/personal) DEĞİL → ilk `slug==='personal'` + `--two` grid yanlıştı; Chrome'da yakalandı (3 plan 3-up + Premium featured + AR rtl doğrulandı). `tsc` 0, `next lint` 0 error (kalan 2 uyarı admin/page.tsx, S27'den, kapsam dışı). **Ders pekişti: rapora değil RUNTIME'a güven — statik diff temizdi ama gerçek veri farklıydı.**
>
