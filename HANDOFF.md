...
> İlk gerçek e-posta 2026-06-11'de teslim edildi (Resend). Smoke: **58/58 yeşil** (S24 sonrası). **Sprint 20–24 KAPANDI.** Yol haritası + bug listesi artık ayrı dosyalarda: **`ROADMAP.md`** (sprint planı), **`BUGS.md`** (yaşayan bug listesi). Sıradaki: **S25** (i18n + ayrı admin) — `SPRINT_25_I18N.md`.
>
...
1. **S25 (SIRADAKİ) — i18n + ayrı admin hesabı.** Login/kayıt + dashboard çok dilli (TR+EN), settings'te dil seçici; ayrı gerçek admin `ilkeronurkaya@gmail.com` (migration `0017`). Tasarım: `SPRINT_25_I18N.md`, ajan promtu: `AGENT_PROMPT_SPRINT_25.md`.
2. Sonra: S26 cookie onayı → S27 SMS (NetGSM) → S28 Google/Apple OAuth (dış hesap, en sona). Bkz. `ROADMAP.md`.
3. Açık buglar: B1 (premium seed sahte hash), B2 (Yeni Paket 0-default), **B3 (next build ESLint borcu — prod deploy bloke)**, B4 (smoke :3000 redirect kırılganlığı). Bkz. `BUGS.md`.
4. Resend domain doğrulama → key rotate · gerçek İyzico sandbox (merchant hesabı) · canlıya çıkış (wrangler deploy + Pages).
